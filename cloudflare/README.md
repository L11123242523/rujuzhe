...
## 协议（与根目录 `online_server.js` 完全一致，前端无需区分后端）
...
1. 注册 Cloudflare（邮箱即可），在仪表盘右侧拿到 **Account ID**；
   在 My Profile → API Tokens 创建令牌，模板选 **Edit Cloudflare Workers**
   （已包含 Workers Scripts 与 Durable Objects 权限）。
2. 配置环境变量后部署：
...
部署成功后得到 `wss://rujuzhe-relay.<你的子域>.workers.dev`。

3. 打开 GitHub Pages 上的 `game.html` → 联机大厅，服务器栏填该 wss 地址；
   房主点“创建房间”得到 4 位房号，把房号发给异地好友，好友输入后“加入房间”。
