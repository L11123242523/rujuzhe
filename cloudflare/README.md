...
> **2026-09-16 收口：服务器权威引擎（方案 C，规则跑在 Durable Object 里）已整条删除。**
> 现在只有一条路：**房主权威** —— 房主浏览器跑规则并推快照，客人只画不跑。
> 本文档此前写的"确定性双机推演"是**更早的架构（镜像锁步）**，已作废，别再照它理解代码。
> 联机部分的唯一说法在仓库根目录 `收口记录.md`。

前端 `game.html` 采用**房主权威**：只有房主跑规则，客人只负责绘制并发意图；
中继**不做任何规则结算**，只负责：建房发号 / 两人就绪后发同一颗权威种子（房主先手）/
原样转发消息 / 暂存房主推来的权威快照（供任一方断线重连）。
